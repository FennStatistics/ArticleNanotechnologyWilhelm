...
dir()
read_file('jatos_results_20210602094601.txt') %>%
  # ... split it into lines ...
  str_split('\n') %>% first() %>%
  # ... filter empty rows ...
...
dir()
dat_prolific <- read.csv(file = "prolific_export_609beefa0a6defb6f10d91ce.csv")
head(dat_prolific)
...
##############
ques_mixed <- c("prolific_pid", "idcodegroup", "loginID", "loginPassword", "idcodefinal", "dummy_informedconsent",
...
for(i in 1:nrow(questionnaire)){
  if(any(dat_prolific$participant_id %in% questionnaire$prolific_pid[i])){
  ...
  questionnaire$sociodemo_employment[i] <- dat_prolific$Employment.Status[dat_prolific$participant_id %in% questionnaire$prolific_pid[i]]
    ...
    print(questionnaire$prolific_pid[i])
    print(questionnaire[questionnaire$prolific_pid == questionnaire$prolific_pid[i],])
  }
}
...
write.xlsx(questionnaire, file = "questionnaire_tp2.xlsx", row.names = FALSE)


write.xlsx(questionnaire[,c(1:6)], file = "report_data_tp2.xlsx", row.names = FALSE)
...
write.xlsx(questionnaire_para, file = "questionnaire_para_tp2.xlsx", row.names = FALSE)
...
write.xlsx(a, file = "questionnaire_combined_tp2.xlsx", row.names = FALSE)
rm(a)
