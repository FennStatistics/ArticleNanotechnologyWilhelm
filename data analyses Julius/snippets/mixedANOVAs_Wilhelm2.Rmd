...
# transform one experimental group to the other
...
dat_long$idcodegroup_sum <- dat_long$idcodegroup 

dat_long$idcodegroup_sum[dat_long$idcodegroup == "positive"] <- "negative"
dat_long %>%
  group_by(idcodegroup_sum, timepoint) %>%
dplyr::summarise(n = n(), mean=mean(mean_valence_macro_alt2), sd=sd(mean_valence_macro_alt2))


### mixed ANOVAs
fit1 <- afex::aov_car(mean_valence_macro_alt2 ~ timepoint*idcodegroup_sum + Error(prolific_pid / timepoint),
                      data = dat_long)

fit1a <- afex::aov_ez(id = "prolific_pid", dv = "mean_valence_macro_alt2",
                      data = dat_long, between=c("idcodegroup_sum"), within=c("timepoint"))
...
dfvalcor <- data_summary(dat_long, varname="mean_valence_macro_alt2",
                         groupnames=c("timepoint","idcodegroup_sum"))
dfvalcor

# create ggplot2 object for X
p <- ggplot(dfvalcor, aes(x=timepoint, y=mean_valence_macro_alt2, fill=idcodegroup_sum)) +
...
table(dat_long_2$idcodegroup)

###pre-post differences
##filtering and ordering
dat_long_2_sub <- dat_long_2 %>%
  select(prolific_pid, timepoint, idcodegroup, mean_valence_macro) %>%
  filter(timepoint == 0) %>%
  filter(str_detect(idcodegroup, "experimental"))
dat_long_2_sub_ordered <- dat_long_2_sub[ order( dat_long_2_sub$prolific_pid), ]
head(dat_long_2_sub_ordered)


dat_long_2_sub2 <- dat_long_2 %>%
  select(prolific_pid, timepoint, idcodegroup, mean_valence_macro) %>%
  filter(timepoint == 1) %>%
  filter(str_detect(idcodegroup, "experimental"))
dat_long_2_sub2_ordered <- dat_long_2_sub2[ order( dat_long_2_sub2$prolific_pid), ]
head(dat_long_2_sub2_ordered)

##computing pre-post differences
cam_diff_eg <- (dat_long_2_sub_ordered$mean_valence_macro -
dat_long_2_sub2_ordered$mean_valence_macro)
head(cam_diff_eg)

#removing negative signs
cam_diff_eg_abs <- abs(cam_diff_eg)
head(cam_diff_eg_abs)
```
